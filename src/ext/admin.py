...
class Admin(Cog):
    async def cog_before_slash_command_invoke(self, inter: disnake.ApplicationCommandInteraction) -> None:
        ...
        if isinstance(inter.channel, disnake.DMChannel):
            raise commands.CheckFailure("This command is not available in DMs")
        assert isinstance((author := inter.author), disnake.Member)
    ...
    async def warn(
        ...
        await inter.edit_original_response(embed=disnake.Embed(title="User warned", description=reason, color=0xFF0000))
